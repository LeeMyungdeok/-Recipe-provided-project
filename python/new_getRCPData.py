...
def Regex(RCP_NM):                                                                            
    ...
    return food_list
# 해당 bin_FoodData.json 읽어 정규화 된 재료 리스트와 매칭하여 재료를 선별한다. (bin_FoodData.json안에는 신뢰성있는 재료가 쌓여있다.)
def BinMatch(RCP_NM):                                                                           
...
def FoodDBMatchFood(RCP_NM):
    ...
    return new_lsit
# trash.json을 통해 재료가 아닌것들을 필터링 작용을 해서 재료에 매칭하지 못한 애들 중 재료만 나오기 위해서 최종 필터링을 걸친다.
...
# 그리고 재료중 매치되지 않는 값들은 최종적으로 재료라고 판단하기에 신뢰성있는 binFoodData.json으로 간다 그 후 사용자가 똑같은 레시피를 사용해도 이제는 그 재료가 필터링이 된다.
def foodListCkeck(RCP_NM, sing_list, *args):
    try:
        ...
        return all_list      
# 만개의 레시피에서 크롤링을 한 후에 Trash.json에서 재료가 아닌 것들을 필터링 작용을 해서 최종 음식이름, 재료, 레시피가 소환된다.
